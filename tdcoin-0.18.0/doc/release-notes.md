Tdcoin Core version 0.18.0 is now available from:

  <https://tdcoincore.org/bin/tdcoin-core-0.18.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://tdcoincore.org/en/list/announcements/join/>
...
`/Applications/Tdcoin-Qt` (on Mac) or `tdcoind`/`tdcoin-qt` (on
Linux).
...
Tdcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Tdcoin Core on unsupported systems.

Tdcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Tdcoin Core does not yet change appearance when
...
`contrib/init/tdcoind.service`, it has been changed to use
`/var/lib/tdcoind` as the data directory instead of
`~tdcoin/.tdcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/tdcoind` will
exist has enough space (check using `df -h /var/lib/tdcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/tdcoin/tdcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://tdcoincore.org/en/contact/) the Tdcoin Core
...
The systemd init file (`contrib/init/tdcoind.service`) has been changed
to use `/var/lib/tdcoind` as the data directory instead of
`~tdcoin/.tdcoin`. This change makes Tdcoin Core more consistent with
...
`datadir` option in `/etc/tdcoin/tdcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/tdcoin/tdcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/tdcoin/tdcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/tdcoin/tdcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/tdcoin/tdcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/tdcoin/tdcoin/blob/master/doc/tdcoin-conf.md)
  about the `tdcoin.conf` file describes how to use it to configure
  Tdcoin Core.

- A new document introduces Tdcoin Core's BIP174 [Partially-Signed
  Tdcoin Transactions
  (PSBT)](https://github.com/tdcoin/tdcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/tdcoin/tdcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Tdcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Tdcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/tdcoin/tdcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/tdcoin/tdcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Tdcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Tdcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/tdcoin/tdcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/tdcoin/tdcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart tdcoind with the `-deprecatedrpc=generate`
...
- A new `tdcoin-wallet` tool is now distributed alongside Tdcoin
...
This section describes planned changes to Tdcoin Core that may affect
other Tdcoin software and services.

- Since version 0.16.0, Tdcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Tdcoin Core 0.20 (expected about
  a year after 0.18), Tdcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Tdcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Tdcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Tdcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Tdcoin Core
...
- A sub-project of Tdcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Tdcoin Core.  See their [project
  page](https://github.com/tdcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Tdcoin Core's own implementation, although entropy
  gathered by Tdcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Tdcoin Core a
...
- On macOS, Tdcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `tdcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libtdcoin_server.a` (jnewbery)
...
- #13926 [Tools] tdcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like tdcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `tdcoin-qt -version` output formatting (hebasto)
...
- #11625 Add TdcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libtdcoin_server` (laanwj)
...
- #15154 Configure: tdcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/tdcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Tdcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libtdcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in tdcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Tdcoin Core (Christewart)
...
- #12246 Bugfix: Only run tdcoin-tx tests when tdcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_tdcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_tdcoin` (MarcoFalke)
...
- #14186 tdcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_tdcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update tdcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/tdcoin-conf.md (hebasto)
...
- #15354 Add missing `tdcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct tdcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support tdcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/tdcoin/).
