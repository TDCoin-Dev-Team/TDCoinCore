...
const char * const TDCOIN_CONF_FILENAME = "tdcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "tdcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Tdcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Tdcoin
    // Mac: ~/Library/Application Support/Tdcoin
    // Unix: ~/.tdcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tdcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tdcoin";
#else
    // Unix
    return pathRet / ".tdcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", TDCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Tdcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Tdcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Tdcoin Core developers";
    }
