...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " tdcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  tdcoin-tx [options] <hex-tx> [commands]  Update hex-encoded tdcoin transaction\n" +
            "or:     tdcoin-tx [options] -create [commands]   Create hex-encoded tdcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded tdcoin transaction
